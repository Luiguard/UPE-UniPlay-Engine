import { AnchorState, EntityFlags } from '../types.js';
...
export class BinarySerializer {
    ...
    public static packStateUpdate(entities: AnchorState[]): Uint8Array {
        const buffer = new ArrayBuffer(4 + entities.length * BYTES_PER_ENTITY); // 4 bytes for count header
        const view = new DataView(buffer);
        let offset = 0;

        // Header: entity count
        view.setUint32(offset, entities.length, true); offset += 4;

        for (const entity of entities) {
            // Entity ID hash
            view.setUint32(offset, this.hashEntityId(entity.entityId), true); offset += 4;
            ...
            // Velocity (3x float32)
            view.setFloat32(offset, entity.velocity.x, true); offset += 4;
            view.setFloat32(offset, entity.velocity.y, true); offset += 4;
            view.setFloat32(offset, entity.velocity.z, true); offset += 4;

            // Rotation: compress 0-360 into 0-255
            view.setUint8(offset, Math.round((entity.rotation / 360) * 255) & 0xFF); offset += 1;

            // Health
            view.setUint8(offset, Math.min(255, Math.max(0, entity.health)) & 0xFF); offset += 1;

            // Flags
            view.setUint8(offset, entity.flags & 0xFF); offset += 1;

            // ObjectState
            view.setUint8(offset, entity.objectState & 0xFF); offset += 1;
        }

        return new Uint8Array(buffer);
    }

    public static unpackStateUpdate(data: Uint8Array): { idHash: number, position: {x:number,y:number,z:number}, velocity: {x:number,y:number,z:number}, rotation: number, health: number, flags: number, objectState: number }[] {
        ...
        for (let i = 0; i < count; i++) {
            const idHash = view.getUint32(offset, true); offset += 4;
            ...
            const rotationByte = view.getUint8(offset); offset += 1;
            const health = view.getUint8(offset); offset += 1;
            const flags = view.getUint8(offset); offset += 1;
            const objectState = view.getUint8(offset); offset += 1;
